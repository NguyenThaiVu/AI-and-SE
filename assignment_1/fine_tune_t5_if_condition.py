...
def make_tokenize_fn(tokenizer: T5Tokenizer, max_src_len: int, max_tgt_len: int):
    def _fn(batch):
        model_inputs = tokenizer(
            batch["source"], truncation=True, max_length=max_src_len
        )
...
def make_compute_metrics(tokenizer: T5Tokenizer):
    def _cmp(eval_pred):
        ...
        exact = sum(
            normalize(p) == normalize(g) for p, g in zip(pred_texts, gold_texts)
        )
...
def finetune_t5_from_list(
    ...
    if not pairs:
        raise ValueError(
            "No (masked, condition) pairs could be built from the provided functions."
        )
...
if __name__ == "__main__":
    ...
    PATH_FILE_DATA = os.path.join(
        os.getcwd(), "dataset", "processed", "processed_data.csv"
    )
    ...
    FUNCTIONS = list_python_function[:N_train]
